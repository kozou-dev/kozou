// Foreign-key relation picker, end-to-end against the in-house @kozou/api
// server (KOZOU_ADAPTER_KIND=api, wired in global-setup). The same Admin UI
// build drives a single-column FK (books.author_id -> authors) through the
// create + edit forms: the relation-select picker loads its options from the
// `/relation-options` endpoint, the create form picks an author, and the
// edit form keeps the existing author so the NOT NULL key is not dropped.
// The loop creates then deletes its row, leaving the three seeded books.
