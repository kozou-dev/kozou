<script lang="ts">
  ...
  function rowKey(row: Record<string, unknown>, idx: number): string {
    const pkField = data.table.primaryKey[0];
...
<form method="GET" class="mb-4 flex flex-wrap items-center gap-2">
  ...
  <a
    href={`/tables/${data.table.qualifiedName}/new`}
...
<div class="overflow-x-auto rounded-md border border-border">
  <table class="min-w-full text-sm">
    ...
    <tbody>
      ...
      {:else}
        {#each data.list.rows as row, idx (rowKey(row, idx))}
          <tr class="border-t border-border">
            ...
            <td class="px-3 py-2">
              <a
                href={`/tables/${data.table.qualifiedName}/${rowKey(row, idx)}`}
