// Playwright globalTeardown for the `kozou dev --adapter api` suite.
...
async function safe(label: string, action: () => Promise<unknown> | unknown) {
  try {
    ...
    console.warn(`[kozou-e2e-api teardown] failed to stop ${label}:`, err);
  }
}

export default async function globalTeardown() {
  console.log('[kozou-e2e-api teardown] stopping services');
  ...
  console.log('[kozou-e2e-api teardown] done');
}
