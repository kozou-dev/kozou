...
```bash
docker pull ghcr.io/kozou-dev/kozou:v1.19.0
docker run --rm ghcr.io/kozou-dev/kozou:v1.19.0 mcp --help
```
...
Runtime requirements for v1.19.0:
...
- v1.17.0 (shipped): **the MCP HTTP endpoint becomes optional, and the surfaces that describe it stop overstating what they know.** Set `server.mcp.http.enabled: false` — or `KOZOU_MCP_HTTP_ENABLED=false`, which both shipped compose stacks forward, since the container path mounts no config file — and `kozou dev` brings up the Admin UI and REST with **no MCP listener and no schema cache** (that cache exists only to serve MCP there, so the introspection client is never opened), `kozou mcp --http` refuses instead of contradicting the config, the Admin UI drops its "Connect an AI agent" header link, dashboard card and page (a direct visit gets a 404 naming the config key), and an `auth` block under a disabled endpoint is rejected as dead configuration rather than one side being silently ignored. Adding authentication needs an authorization server; declining to run an endpoint needs nothing, so for a runtime that only ever serves the Admin UI this is the cheaper posture. Alongside it, three classes of honest-diagnostics fix. (a) **When the accepted token contract is not the advertised one**, startup now says so: `jwt.issuer` / `jwt.audience` decide what is honoured while the RFC 9728 metadata decides what is advertised, and a WARNING covers the shapes nobody wants (an accepted issuer that was never advertised, an advertised server whose spelling of `iss` is not accepted, an audience reaching beyond the advertised resource, and the degenerate constraints) while a deliberate NOTE — not a warning — covers the documented audience escape hatch, so a deployment that followed the guide does not boot into a permanent warning. (b) **The "Connect an AI agent" page now describes the endpoint it actually has.** Its authentication wording binds to a posture the CLI injects on every run instead of asserting "no authentication" as static text (that sentence was wrong for an OAuth-protected endpoint, and it is the sentence an operator uses to decide whether the port is safe to expose); in the OAuth posture it hands out the **canonical `resource` URI** rather than a URL rebuilt from the browser's Host header, which was wrong for exactly the proxy or tunnel deployment that `auth.resource` exists for — the printed URL, the `mcpServers` JSON and the `claude mcp add` command all of them; it states no tool count (it had said "seven" since `search_schema` made it eight, a hardcoded fact about a package it does not depend on); and it makes no claim about the bind host (both compose stacks set `0.0.0.0`, so the page called loopback what the container's own stderr warned about) — only that without authentication, whatever can reach the port can read the schema. (c) **A refused config prints what it says.** The issues `KozouConfigError` already carried — the field path and the reason — are rendered instead of the bare `1 issue(s)` count, with a location line reporting the file *and* the environment variables a value was actually taken from (`loaded from: /srv/app/kozou.config.yaml, with values from KOZOU_MCP_HTTP_ENABLED`), so an environment-caused fault is no longer stamped with a file that says nothing about it; an absent `database` block now lands on `database.url` with the message naming `DATABASE_URL`, instead of an object-level "expected object, received undefined". Plus supply-chain work: the blocking CVE gate now scans **the trees an adopter installs**, reconstructed from each publishable package's declared ranges with `pnpm.overrides` excluded — workspace overrides never reach a consumer's resolver, so the previous root-lockfile scan simultaneously over-reported (dev tooling and examples gated merges as hard as published code) and under-reported (a real consumer-side advisory was invisible behind an override) — 19 advisories cleared by in-range bumps and override floors, every scanner action pinned by digest with CodeQL moved to v4, and a report-only scan of the published image's base layer.
- v1.18.0 (shipped): **the "Connect an AI agent" page can be told where the endpoint is actually reached, and the refusal that guards that value stops describing itself loosely.** `server.mcp.http.advertisedUrl` — or `KOZOU_MCP_HTTP_ADVERTISED_URL`, which both shipped compose stacks forward, since the container path mounts no config file — declares the address clients register, and the page hands it out verbatim instead of rebuilding one from the browser's Host header and the port the runtime *binds*. Those are two different addresses the moment anything sits in between (a remapped published port, a tunnel, a devcontainer, a proxy), and until now the page could not be told: it handed out an address that was not the endpoint — nothing behind it, or whatever took the port after a clash. The value is refused rather than silently resolved when it contradicts something: alongside an `auth` block (clients discover `resource` from the endpoint's own metadata, so a second declared address would send the operator elsewhere), under a disabled endpoint, carrying a query or fragment, on a scheme other than http or https, or on a path that is not exactly `/mcp`, the path the transport serves. That last refusal announced itself as "must end in /mcp" while the check is exact equality, so `https://host/api/mcp` — which does end in /mcp — was turned away by a rule it satisfies, and editing against the message returned the same error. It now states the predicate it applies and names what that refuses, including the capability limit the loose wording hid: **an endpoint reached under a path prefix cannot be advertised**. The same claim had shipped further than the message — both `.env.example` files described the value as "ending in /mcp", and that file is scaffolded into every new project, while both compose templates and both READMEs offered "a proxy" among the deployments to use it for without saying the path must survive unchanged, and implied a free choice of scheme. Guards now hold the parts that can be held by shape rather than by prose: every address a refusal offers as the way out must itself pass the validator, and no shipped stack may document a URL the validator refuses — with the limits of those guards recorded beside them, since prose polarity and paraphrase are not among them. Alongside, corrections to what the project says about REST: `kozou dev` binds `@kozou/api` to loopback with a hardcoded host and the scaffolded compose stack publishes no REST port at all, so serving REST to your own clients is a thing you do rather than a thing the stack does; a loopback bind constrains where a caller is, not who it is, which the source comments had turned into "reached only by the Admin UI" — this repo's own `examples/react` points a separate app at that port; and the README now points at the page that says whether to run this or the desktop app. Plus the `@hono/node-server` CVE exception, dropped because the advisory's fix reached the 1.x line the consumer trees resolve to, and the blocking gate refuses to let a stale ignore sit quietly.
- v1.19.0 (shipped): **the row-level-security signal now names the commands RLS refuses outright**, additive. `hasPolicies` is per table while RLS is enforced per command, so a table could report `enabled`, report `hasPolicies`, hold the GRANT — and still refuse every INSERT, because the only policy written was `FOR SELECT`. Everything the agent received was accurate, and none of it predicted the refusal. MCP `describe_table` and `kozou docs` now carry **`rowSecurity.deniedCommands`**: the commands no permissive policy can cover, in a fixed order, empty while RLS is off. It is derived from `pg_policy.polcmd` and `polpermissive` and nothing else — `FOR ALL` is stored as `polcmd = '*'` and counts for every command, and a restrictive policy grants nothing, so a command whose only policy is restrictive is still default-deny — which means **no policy expression is read**, and the roles a policy names are deliberately not consulted either, so the answer is the same for every caller and stays safe to compile once and hand to anyone. That makes the signal **sound in one direction only, and it is the safe one**: a listed command IS refused for every role RLS applies to, whatever privileges have been granted; a command *not* listed may still be refused — by a policy scoped to other roles, or by a `USING` expression that matches nothing. It never promises a write will succeed, and the types say so. The advisory note also names the two shapes a refusal takes, because only one of them is visible: an INSERT raises an error once it writes a row, while a SELECT, UPDATE or DELETE — and an INSERT that writes none, since the check runs per written row — simply matches nothing, which a caller cannot tell apart from "there was nothing to change". The three ways a per-command *count* would have read wrong — `FOR ALL`, a restrictive policy, and a policy naming other roles — are each measured against a real PostgreSQL server with a non-owner role, as are both shapes of refusal; the incompleteness is measured rather than argued. The field is required on the compiled context type and optional on the MCP output schema, so a context compiled before it existed still reads. Alongside it, the guarantee this sits on top of — Kozou reads *whether* a row-security policy exists, never *what it says* — stops being carried by a comment: a canary string that exists only inside a policy body is swept for across everything Kozou emits (raw introspection, compiled context, all eight MCP tools, `kozou docs`, the OpenAPI document), and a source assertion fails if the introspection SQL ever names the catalog columns that would expose an expression — with the limits of both guards recorded beside them.
- Beyond v1.19: React UI exploration — optional write-path parity (a second UI driving create / edit / delete across both adapters)
